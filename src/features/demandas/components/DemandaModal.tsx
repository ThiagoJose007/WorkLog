...
export function DemandaModal({ demanda, projetoId, isOpen, onClose }: DemandaModalProps) {
  const isEditing = !!demanda

  const [activeTab, setActiveTab] = useState<ModalTab>('detalhes')
  ...
  // Sync TipTap content when modal opens (editor may not be ready on first render)
  useEffect(() => {
    if (isOpen && editor) {
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          {/* Tabs — visíveis apenas no modo edição */}
          {isEditing && (
            <div
            ...
            >
              {([
                { id: 'detalhes' as ModalTab, label: 'Detalhes' },
                { id: 'timer' as ModalTab, label: 'Timer & Histórico', icon: Clock },
          ...
          <div className="p-6 space-y-5">
                ...
                <div className="flex gap-2 flex-wrap">
                  {TIPO_OPTIONS.map((t) => (
                    <button
                      ...
                      onClick={() => setTipo(t)}
                      style={{
                        outline: tipo === t ? '2px solid var(--accent)' : 'none',
                        outlineOffset: '2px',
                        borderRadius: '6px',
                        opacity: tipo === t ? 1 : 0.5,
                      }}
                    >
              ...
              <div className="flex gap-3 flex-wrap">
                {PRIORIDADE_OPTIONS.map((p) => (
                  <button
                    ...
                    style={{
                      outline: prioridade === p ? '2px solid var(--accent)' : 'none',
                      outlineOffset: '2px',
                      borderRadius: '6px',
                      opacity: prioridade === p ? 1 : 0.45,
                    }}
                  >
            ...
            <div className="space-y-1.5">
              ...
              <div
                className="rounded-md overflow-hidden tiptap-wrapper"
                style={{ border: '0.5px solid var(--border)', backgroundColor: 'var(--bg-input)' }}
              >
                {/* Toolbar */}
                <div
                ...
                >
                  {[
                    ...
                    <button
                      ...
                      style={{
                        color: editor?.isActive(name) ? 'var(--accent)' : 'var(--text-muted)',
                  ...
                  {pendingLinks.map((link, i) => (
                    ...
                    >
                      <span
                        ...
                        style={{
                          backgroundColor: FERRAMENTA_COLOR[link.ferramenta] + '22',
                          color: FERRAMENTA_COLOR[link.ferramenta],
                          border: `0.5px solid ${FERRAMENTA_COLOR[link.ferramenta]}44`,
                        }}
                      >
                      ...
                      <a
                        ...
                        className="flex-1 min-w-0 text-xs truncate flex items-center gap-1 hover:underline"
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors hover:bg-[var(--bg-surface)]"
                style={{
